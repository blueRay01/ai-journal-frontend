...
function UncheckDialog({ title, onConfirm, onCancel }) {
  return (
      ...
      <div className="relative bg-white rounded-2xl shadow-xl p-6 w-[300px] flex flex-col gap-4 z-10">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-[#888880] leading-relaxed">
            This task was completed on time. Unchecking it will remove it from your completed entries.
            If you re-check it, it will be marked as <span className="text-[#c05050] font-medium">late</span>.
          </p>
        </div>
...
export default function TimelineProgressionCard() {
  ...
  // wasLate: tracks if a re-check should always show late
  ...
  const [confirmDialog, setConfirmDialog] = useState(null); // { id, title } | null
  ...
  const handleCircleClick = (entry) => {
    const isPast = toMinutes(entry.time) < currentTime;
    ...
    if (isChecked && !isLate) {
      // Was completed on time — ask before unchecking
      setConfirmDialog({ id: entry.id, title: entry.title });
    } else if (isChecked && isLate) {
      // Late entry being unchecked — just uncheck, stays permanently late on re-check
      setChecked((prev) => ({ ...prev, [entry.id]: false }));
    } else {
      // Unchecked → checking it now
      // If time has passed (or it was previously unchecked after being on-time), mark as late
      const shouldBeLate = isPast || wasLate[entry.id];
  ...
  const confirmUncheck = () => {
    ...
    // Mark as "will be late on re-check" since time has already passed
